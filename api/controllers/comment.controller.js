...
export const likeComment = async (req, resp, next) => {
  try {
...
export const editComment = async (req, resp, next) => {
  try {
    const comment = await Comment.findById(req.params.commentId);
    if (!comment) {
      return next(errorHandler(404, "Comment not found"));
...
export const getComments = async (req, resp, next) => {
  if (!req.user.isAdmin) {
  ...
  try {
    ...
    resp.status(200).json({ comments, totalComments, lastMonthComments });
  } catch (error) {
    next(error);
  }
};
